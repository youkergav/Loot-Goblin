...
func drop_data_at_global_position(at_position: Vector2, item_slot: Variant) -> void:
    var item_data = item_slot.item_data
    
    # Remove from queue and shift everything left
    hotbar.remove_item_and_shift(item_slot)
    spawn_item_in_world_at_global_position(item_data, at_position)
    
func spawn_item_in_world_at_global_position(item_data: ItemData, world_position: Vector2) -> void:	
